...
class PtxService:
    ...
    def _row_to_rec(self, row) -> PtxRecord:
        return PtxRecord(
            ptx_key=row["ptx_key"],
    ...
    def read_by_key(self, ptx_key: str):
        try:
    ...
    def read_lock(self, ptx_key: str):
        try:
            with self._conn().cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute(
                    "SELECT ptx_key, ptx_code1, ptx_desc, ptx_subdesc, ptx_other_data "
    ...
    def read_by_alt_key(self, ptx_code1: str, ptx_desc: str = ""):
        try:
            with self._conn().cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute(
                    "SELECT ptx_key, ptx_code1, ptx_desc, ptx_subdesc, ptx_other_data "
        ...
        try:
            with self._conn().cursor() as cur:
                cur.execute(
                    ...
                    "VALUES (%s, %s, %s, %s, %s)",
                    (_pad(rec.ptx_key, 12), _zpad(rec.ptx_code1, 2),
                     _pad(rec.ptx_desc, 25), _pad(rec.ptx_subdesc, 25),
        ...
        try:
            with self._conn().cursor() as cur:
                cur.execute(
                    ...
                    (_zpad(rec.ptx_code1, 2), _pad(rec.ptx_desc, 25),
                     _pad(rec.ptx_subdesc, 25), _pad(rec.ptx_other_data, 700),
                     _pad(rec.ptx_key, 12))
                )
