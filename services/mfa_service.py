...
All I/O for the MFA file — replaces ISAM operations from selmfa.cp / recmfa.cp
"""
import psycopg2
...
class MfaService:
    ...
    def _row_to_rec(self, row) -> MfaRecord:
        return MfaRecord(
            mfa_gan=row["mfa_gan"],
    ...
    def read_by_key(self, mfa_gan: str):
        try:
            with self._conn().cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT * FROM mfa WHERE mfa_gan = %s",
                            (mfa_gan.ljust(12)[:12],))
                row = cur.fetchone()
            if row:
                self.file_status = "00"
    ...
    def read_lock(self, mfa_gan: str):
        try:
            with self._conn().cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT * FROM mfa WHERE mfa_gan = %s FOR UPDATE NOWAIT",
                            (mfa_gan.ljust(12)[:12],))
                row = cur.fetchone()
            if row:
                self.file_status = "00"
